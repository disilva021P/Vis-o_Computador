...
static bool processaBlob(cv::Mat& frame, const OVC& blob, int nframe,
    IVC* imageBGR, IVC* imageDilated, IVC* imageMask,
    ...
    float diamMin, float diamMax)
{
    ...
    if (pct_defeito < 0.05f) pontos += 1;
    else if (pct_defeito < 0.15f) pontos += 2;
    else if (pct_defeito < 0.30f) pontos += 3;
    else                          pontos += 4;
...
static float calcDefeito(IVC* imageBGR, const OVC& blob)
{
    unsigned char* data = imageBGR->data;
    int bpl = imageBGR->bytesperline;
    int ch = imageBGR->channels;
    int imgW = imageBGR->width;
    int imgH = imageBGR->height;

    float rx = (blob.width / 2.0f) * 0.80f;
    float ry = (blob.height / 2.0f) * 0.80f;
    float cx = (float)blob.xc;
    float cy = (float)blob.yc;
    ...
    long int pixelsDentro = 0;
    long int pixelsDefeito = 0;
    ...
    for (int y = y0; y < y1; y++) {
        for (int x = x0; x < x1; x++) {
            ...
            int pos = y * bpl + x * ch;
            float B = (float)data[pos];
            float G = (float)data[pos + 1];
            float R = (float)data[pos + 2];

            
            
            
            float ratioG = (R > 0) ? G / R : 1.0f;
            float ratioB = (R > 0) ? B / R : 1.0f;


            
            bool corAnormal = (ratioG > 0.55f || ratioB > 0.25f);

            
            bool muitoEscuro = (R < 80 && G < 50 && B < 50);


            if (muitoEscuro || corAnormal)
                pixelsDefeito++;
        }
    }

    if (pixelsDentro == 0) return 0.0f;
    return (float)pixelsDefeito / (float)pixelsDentro;
}
    ...
    float diamMinFrame, float diamMaxFrame)
{
    ...
    const char* cat = "?";
    if (!jaContado) {
        pct_defeito = calcDefeito(imageBGR, blob);
    ...
    int draw_bx = infoCongelado ? infoCongelado->cx - draw_bw / 2 : blob.x;
    int draw_by = infoCongelado ? infoCongelado->cy - draw_bh / 2 : blob.y;
    ...
    while (key != 'q') {
        ...
        if (blobs != NULL && nlabels > 0) {
            ...
            for (int i = 0; i < nlabels; i++) {
                ...
                float d = calcDiametroMM(blobs[i].width, blobs[i].height);
                if (d >= DIAM_MIN_MM) {
                    if (processaBlob(frame, blobs[i], nframe,
                        imageBGR, imageEroded, imageMask,
